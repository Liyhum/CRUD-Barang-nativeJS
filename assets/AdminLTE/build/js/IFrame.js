...
class IFrame {
  ...
  createTab(title, link, uniqueName, autoOpen) {
    ...
    const newNavItem = `<li class="nav-item" role="presentation"><a href="assets/AdminLTE/#" class="btn-iframe-close" data-widget="iframe-close" data-type="only-this"><i class="fas fa-times"></i></a><a class="nav-link" data-toggle="row" id="${navId}" href="assets/AdminLTE/#${tabId}" role="tab" aria-controls="${tabId}" aria-selected="false">${title}</a></li>`
    $(SELECTOR_TAB_NAVBAR_NAV).append(unescape(escape(newNavItem)))

    const newTabItem = `<div class="tab-pane fade" id="${tabId}" role="tabpanel" aria-labelledby="${navId}"><iframe src="assets/AdminLTE/${link}"></iframe></div>`
