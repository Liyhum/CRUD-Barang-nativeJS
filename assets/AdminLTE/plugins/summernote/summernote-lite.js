          ...
          value = value.replace(/(<iframe.*?>.*?(?:<\/iframe>)?)/gi, function (tag) {
            ...
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done;) {
                ...
                if (new RegExp('src="assets/AdminLTE/(https?:)?\/\/' + src.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&') + '\/(.+)"').test(tag)) {
                  return tag;
                }
              }
            } catch (err) {
...
var HelpDialog = /*#__PURE__*/function () {
  ...
  HelpDialog_createClass(HelpDialog, [{
    ...
    value: function initialize() {
      var $container = this.options.dialogsInBody ? this.$body : this.options.container;
      var body = ['<p class="text-center">', '<a href="assets/AdminLTE/http://summernote.org/" target="_blank" rel="noopener noreferrer">Summernote 0.8.20</a> · ', '<a href="assets/AdminLTE/https://github.com/summernote/summernote" target="_blank" rel="noopener noreferrer">Project</a> · ', '<a href="assets/AdminLTE/https://github.com/summernote/summernote/issues" target="_blank" rel="noopener noreferrer">Issues</a>', '</p>'].join('');
      this.$dialog = this.ui.dialog({
...
var dropdown = renderer.create('<div class="note-dropdown-menu" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    ...
    var $temp = external_jQuery_default()('<a class="note-dropdown-item" href="assets/AdminLTE/#" data-value="' + value + '" role="listitem" aria-label="' + value + '"></a>');
...
var dropdownCheck = renderer.create('<div class="note-dropdown-menu note-check" role="list"></div>', function ($node, options) {
  var markup = Array.isArray(options.items) ? options.items.map(function (item) {
    var value = typeof item === 'string' ? item : item.value || '';
    var content = options.template ? options.template(item) : item;
    var $temp = external_jQuery_default()('<a class="note-dropdown-item" href="assets/AdminLTE/#" data-value="' + value + '" role="listitem" aria-label="' + item + '"></a>');
...
var videoDialog = function videoDialog(opt) {
  ...
  var footer = ['<button type="button" href="assets/AdminLTE/#" class="note-btn note-btn-primary note-video-btn disabled" disabled>', opt.lang.video.insert, '</button>'].join('');
  return dialog({
...
var imageDialog = function imageDialog(opt) {
  ...
  var footer = ['<button href="assets/AdminLTE/#" type="button" class="note-btn note-btn-primary note-btn-large note-image-btn disabled" disabled>', opt.lang.image.insert, '</button>'].join('');
  return dialog({
...
var linkDialog = function linkDialog(opt) {
  ...
  var footer = ['<button href="assets/AdminLTE/#" type="button" class="note-btn note-btn-primary note-link-btn disabled" disabled>', opt.lang.link.insert, '</button>'].join('');
  return dialog({
    className: 'link-dialog',
